#!/usr/bin/env node
/**
 * Generates src/lib/data/map/commune-lookup.json from the filtered GeoJSON,
...
const sruData = JSON.parse(readFileSync(join(root, 'src/lib/data/map/sru-communes.json'), 'utf8'));
const sruCodes = new Set(Object.keys(sruData));
console.log(`SRU communes: ${sruCodes.size}`);

// Load the filtered runtime fallback generated by npm run build-map-fallbacks.
const geojson = JSON.parse(
  readFileSync(join(root, 'static/communes_2022_sru.geojson'), 'utf8'),
);
...
const outPath = join(root, 'src/lib/data/map/commune-lookup.json');

	...
	const parisProjects = [
		{
			...
			].map((id) => ({ src: `/images/optimized/project-1/${id}-full.webp`, alt: 'Samaritaine' }))
		},
		{
			label: 'Maréchal Fayolle',
			images: ['DSC01776', 'DSC01779', 'DSC01780', 'DSC01794', 'DSC01796', 'DSC01797', 'DSC01798', 'DSC01799'].map(
				(id) => ({ src: `/images/optimized/project-2/${id}-full.webp`, alt: 'Maréchal Fayolle' })
			)
		},
		{
			label: 'Rue Jean-Bart',
			images: ['paris-01', 'DSC01781', 'DSC01783'].map((id) => ({
		...
		},
		{
			...
			images: [
				...
				'laguiole-291',
				'laguiole-297'
			].map((id) => ({ src: `/images/optimized/project-4/${id}-full.webp`, alt: 'Tour BLP' }))
		}
	];

	const brittanyImages = ['brittany-01', 'brittany-02', 'brittany-04'].map((id) => ({
		src: `/images/optimized/brittany/${id}-full.webp`,
		alt: 'Brittany'
	}));

	const regions = ['Paris', 'Brittany', 'French Riviera', 'Overseas Territories'];
</script>
...
<section id="socio-econometrics" class="py-12">
	...
	<div class="max-w-3xl mx-auto mt-8 px-4 sm:px-6">
		<nav class="flex flex-wrap gap-x-6 gap-y-2 text-sm">
			{#each regions as region (region)}
				<a href="#{slug(region)}" class="text-gray-700 underline-offset-4 hover:underline">
					{region}
				</a>
			{/each}
		</nav>
	...
	<div id={slug('Paris')} class="max-w-3xl mx-auto mt-12 px-4 sm:px-6 scroll-mt-24">
	...
	<div class="mt-6 space-y-12">
		{#each parisProjects as project (project.label)}
			<div class="max-w-3xl mx-auto px-4 sm:px-6">
				...
				<p class="mt-4 text-gray-700">{LOREM}</p>
			</div>
		{/each}
	</div>

	<div id={slug('Brittany')} class="max-w-3xl mx-auto mt-16 px-4 sm:px-6 scroll-mt-24">
	...
	<div id={slug('French Riviera')} class="max-w-3xl mx-auto mt-16 px-4 sm:px-6 scroll-mt-24">
		<h1 class="text-4xl font-bold">French Riviera</h1>
		<p class="mt-4 text-gray-700">{LOREM}</p>
	</div>

	<div id={slug('Overseas Territories')} class="max-w-3xl mx-auto mt-16 px-4 sm:px-6 scroll-mt-24">
